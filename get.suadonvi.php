...
<h2>Sửa địa chỉ </h2>
<form action="#" id="suadonviform" onsubmit="return false;">
	...
	<div class="grid">
		<div class="row cells12">
			<div class="cell colspan6 input-control text">
				<input type="text" name="tendonvi" id="tendonvi" value="<?php echo isset($tendonvi) ? $tendonvi : ''; ?>" class="edit-text" size="50" required oninvalid="InvalidMsg(this);" oninput="InvalidMsg(this);" placeholder="Tên địa chỉ"/>
			</div>
			<div class="cell colspan6 input-control select">
				<select name="parent" id="parent" class="donvi" disabled>
					<option value="">Cấp trên</option>
					<?php
						$donvi_list = $donvi->get_all_list();
						if($donvi_list){
							foreach ($donvi_list as $k1 => $a1) {
								echo '<option value="2__'.$a1['_id'].'__'.$k1.'"'.(($level==2 && $a1['_id']==$id_parent) ? ' selected' : '').'>'.$a1['ten'].'</option>';
								if($a1['level2']){
									foreach ($a1['level2'] as $k2 => $a2) {
										echo '<option value="3__'.$a2['_id'].'__'.$k1.'__'.$k2.'"'.(($level==3 && $a2['_id']==$id_parent) ? ' selected' : '').'>&nbsp;&nbsp;&nbsp;-'.$a2['ten'].'</option>';
										if($a2['level3']){
											foreach ($a2['level3'] as $k3 => $a3) {
												echo '<option value="4__'.$a3['_id'].'__'.$k1.'__'.$k2.'__'.$k3.'"'.(($level==4 && $a3['_id']==$id_parent) ? ' selected' : '').'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;--'.$a3['ten'].'</option>';
											}
										}
									}
								}
		...
		<div class="row cells12 align-center">
			<div class="cell colspan12">
				<a href="#" id="update_donvi" class="button primary fg-white" onclick="return false;"><span class="mif-checkmark"></span> Cập nhật</a>
				<a href="#" id="delete_donvi" class="button alert fg-white" onclick="return false;"><span class="mif-bin"></span> Xoá</a>		
			</div>
		</div>
	</div>
</form>
