...
if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
	if(isset($_FILES['dinhkem_baocao']['name']) && $_FILES['dinhkem_baocao']['name']){
		// Loop $_FILES to exeicute all files
		foreach ($_FILES['dinhkem_baocao']['name'] as $f => $name) {   
		    if ($_FILES['dinhkem_baocao']['error'][$f] == 4) {
		        echo 'Failed';
		        continue; // Skip file if any error found
		    } 
		    if ($_FILES['dinhkem_baocao']['error'][$f] == 0) {	           
		        if ($_FILES['dinhkem_baocao']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["dinhkem_baocao"]["tmp_name"][$f], $target_files.$alias_name))
		        	...
		        	echo '<div class="row cells12" style="padding:0px 0px 10px 0px;margin:0px;">';
						...
						echo '<div class="cell colspan10 input-control text">';
							echo '<input type="hidden" name="baocao_alias_name[]" value="'.$alias_name.'" readonly/>';
							echo '<input type="hidden" name="baocao_filetype[]" value="'.$extension.'" readonly/>';
							echo '<span class="mif-attachment prepend-icon"></span>';
							echo '<input type="text" name="baocao_filename[]" value="'. $name.'" class="bg-grayLighter" style="padding-left:50px;"/>';
