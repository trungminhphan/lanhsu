...
if($id && $act=='del' && $users->is_admin()){
	...
	if($doanvao->delete()){
		if($dv['congvanxinphep']['attachments']){
			foreach ($dv['congvanxinphep']['attachments'] as $key => $value) {
				@unlink($target_files . $value['alias_name']);
			}
		}
		if($dv['congvanduocphep']['attachments']){
			foreach ($dv['congvanduocphep']['attachments'] as $key => $value) {
				@unlink($target_files . $value['alias_name']);
			}
		}
		if($dv['congvanchophep']['attachments']){
			foreach ($dv['congvanchophep']['attachments'] as $key => $value) {
				@unlink($target_files . $value['alias_name']);
			}
		}
		transfers_to('doanvao.php?update=ok');
	} else {
		transfers_to('doanvao.php?update=no');
	}
}
switch ($update) {
...
if(isset($_POST['submit'])){
	...
	if($id){
		...
		$doanvao->id = $id;
		if($doanvao->edit()){
			transfers_to('chitietdoanvao.php?id='.$id.'&update=edit_ok');
		} else {
			transfers_to('themdoanvao.php?id='.$id.'&update=edit_no');
		}
	} else {
		//insert
		$id = new MongoId();
		$doanvao->id = $id;
		if($doanvao->insert()){
			transfers_to('chitietdoanvao.php?id='.$id.'&update=insert_ok');
		} else {
			transfers_to('themdoanvao.php?id='.$id.'&update=insert_no');
		}
	}
}
...
<h1><a href="doanvao.php" class="nav-button transform"><span></span></a>&nbsp;Cập nhật thông tin Đoàn Vào</h1>
...
<div class="grid">
	...
	<div class="row cells12">
		...
		<div class="cell input-control text">
			<input type="text" name="tenquyetdinhchophep_2" id="tenquyetdinhchophep_2" value="<?php echo isset($tenquyetdinhchophep_2) ? $tenquyetdinhchophep_2 : ''; ?>" placeholder="Số văn bản cho phép" />
			<span class="input-state-error mif-warning"></span><span class="input-state-success mif-checkmark"></span>
		</div>
		<div class="cell input-control text" data-role="datepicker" data-format="dd/mm/yyyy">
