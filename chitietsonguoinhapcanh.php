...
$donvi = new DonVi();$canbo = new CanBo();$doanra = new DoanVao();
...
?>
<h1><a href="index.php" class="nav-button transform"><span></span></a>&nbsp;Chi tiết số người nhập cảnh</h1>
...
<table class="table border bordered striped hovered">
    ...
    foreach ($songuoi_list as $key => $value) {
        ...
        echo '<tr>
            <td>'.$i.'</td>
            <td>'.$cb['hoten'].'</td>
            <td class="align-right"><a href="thongkedoanvaotheocanhan.php?id_canbo='.$value.'&'.$_SERVER['QUERY_STRING'].'" target="_blank">'.$count.'</a></td>
        </tr>';$i++;
    }
    ?>
    </tbody>
