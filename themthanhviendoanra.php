...
$doanra = new DoanRa();$donvi = new DonVi();$canbo = new CanBo();$chucvu=new ChucVu();
$ham = new Ham();$mucdich=new MucDich();
$id = isset($_GET['id']) ? $_GET['id'] : '';
$act = isset($_GET['act']) ? $_GET['act'] : '';
$key = isset($_GET['key']) ? $_GET['key'] : 0;
if($id && $act == 'del1'){
	$doanra->id = $id; 
	if($doanra->unset_danhsachdoan('danhsachdoan', $key) && $doanra->pull_danhsachdoan('danhsachdoan', $key)){
		transfers_to('themthanhviendoanra.php?id=' . $id);
	}
}
if($id && $act == 'del2'){
	$doanra->id = $id; 
	if($doanra->unset_danhsachdoan('danhsachdoan_2', $key) && $doanra->pull_danhsachdoan('danhsachdoan_2', $key)){
		transfers_to('themthanhviendoanra.php?id=' . $id);
	}
}
$doanra->id = $id; $dv = $doanra->get_one();
if(isset($dv['id_dmdoanra']) && $dv['id_dmdoanra']){
	$dmdoanra->id = $dv['id_dmdoanra']; $d = $dmdoanra->get_one();
	$tendoanra = $d['ten'];
} else { $tendoanra = ''; }
...
if(isset($_POST['submit'])){
	...
	if($doanra->push_danhsachdoan($loaidanhsachdoan)) transfers_to('themthanhviendoanra.php?id=' . $id);
}
...
if(isset($_POST['sort'])){
	...
	if($id_danhsachdoan){
		$doanra->unset_danhsachdoan_all('danhsachdoan');
		foreach($id_danhsachdoan as $key => $value){
			...
			$doanra->danhsachdoan = $danhsachdoan;
			$doanra->push_danhsachdoan('danhsachdoan');
		}
	}
	if($id_danhsachdoan_2){
		$doanra->unset_danhsachdoan_all('danhsachdoan_2');
		foreach($id_danhsachdoan_2 as $key2 => $value2){
			...
			$doanra->push_danhsachdoan('danhsachdoan_2');
		}
	}
	transfers_to('themthanhviendoanra.php?id=' . $id);	
}
...
<h1><a href="chitietdoanra.php?id=<?php echo $id; ?>" class="nav-button transform"><span></span></a>&nbsp;Thêm thành viên Đoàn ra</h1>
...
<hr />
<h3 class="align-center fg-red">Tên đoàn: <?php echo $tendoanra; ?></h3>
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="POST" accept-charset="utf-8">
<h3>Danh sách 1</h3>
	...
	if($dv['danhsachdoan']){
		...
		foreach ($dv['danhsachdoan'] as $k => $ds) {
			...
			echo '<tr>
				...
				<td>'.$cb['hoten'].'</td>
				<td>'.$tenchucvu.'</td>
				<td class="align-center"><a href="themthanhviendoanra.php?id='.$id.'&key='.$k.'&act=del1" onclick="return confirm(\'Chắc chắn muốn xoá?\');"><span class="mif mif-bin"></span></a></td>
			</tr>';	$i++;
		}
	}
?>
	...
	if(isset($dv['danhsachdoan_2']) && $dv['danhsachdoan_2']){
		foreach ($dv['danhsachdoan_2'] as $k2 => $dv_2) {
			...
			echo '<tr>
				...
				<td>'.$cb['hoten'].'</td>
				<td>'.$cv['ten'].'</td>
				<td class="align-center"><a href="themthanhviendoanra.php?id='.$id.'&key='.$k2.'&act=del2" onclick="return confirm(\'Chắc chắn muốn xoá?\');"><span class="mif mif-bin"></span></a></td>
			</tr>';	$i++;
		}	
	}
	?>
